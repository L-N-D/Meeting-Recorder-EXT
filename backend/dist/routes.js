...
const CHUNKS_DIR = path_1.default.resolve(__dirname, '../../uploads/chunks');
const ASSEMBLED_DIR = path_1.default.resolve(__dirname, '../../uploads/assembled');
fs_1.default.mkdirSync(CHUNKS_DIR, { recursive: true });
fs_1.default.mkdirSync(ASSEMBLED_DIR, { recursive: true });
...
router.get('/recordings/health', (req, res) => {
    res.json({ status: 'ok' });
});
...
router.post('/recordings', async (req, res) => {
    try {
        ...
        let rec = await database_1.Recording.findByPk(sessionId);
        if (!rec) {
            rec = await database_1.Recording.create({
                sessionId,
                platform,
...
router.post('/recordings/:id/chunks', upload.single('file'), async (req, res) => {
    try {
        ...
        // A. Check Idempotency
        const existingChunk = await database_1.RecordingChunk.findByPk(idempotencyKey);
        if (existingChunk) {
        ...
        const chunkFilePath = path_1.default.join(CHUNKS_DIR, chunkFileName);
        fs_1.default.writeFileSync(chunkFilePath, file.buffer);
        ...
        const chunk = await database_1.RecordingChunk.create({
...
router.post('/recordings/:id/complete', async (req, res) => {
    try {
        const sessionId = req.params.id;
        const rec = await database_1.Recording.findByPk(sessionId);
        if (!rec) {
            return res.status(404).json({ error: 'Recording session not found' });
        }
        // Load all chunks sorted by index
        const chunks = await database_1.RecordingChunk.findAll({
            where: { sessionId },
        ...
        const finalFilePath = path_1.default.join(ASSEMBLED_DIR, finalFileName);
        const writeStream = fs_1.default.createWriteStream(finalFilePath);
        for (const chunk of chunks) {
            if (fs_1.default.existsSync(chunk.filePath)) {
                const data = fs_1.default.readFileSync(chunk.filePath);
...
router.get('/recordings/:id/status', async (req, res) => {
    try {
        const sessionId = req.params.id;
        const rec = await database_1.Recording.findByPk(sessionId, {
            include: [{ model: database_1.RecordingChunk, as: 'chunks' }]
        });
        if (!rec) {
